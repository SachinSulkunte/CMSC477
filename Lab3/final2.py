...
if __name__ == '__main__':
    ...
    START = "LEFT" # bottom half of map

    while(True):
        wait() # wait a bit
        if START == "LEFT":
            resetArmPos()
            ep_chassis.move(x=0, y=-0.3, z=0, xy_speed=0.5).wait_for_completed() # move left 20cm
            ep_chassis.move(x=0.3, y=0, z=0, xy_speed=0.5).wait_for_completed() # move forward 30cm
            findLine() # find blue line
            driveToLine()
            findLego(model)
            driveToLego(model)
            grabLego() # get lego
            time.sleep(1)
            ep_chassis.move(x=-0.3, y=0, z=0, xy_speed=0.5).wait_for_completed() # move back 30cm
            ep_chassis.move(x=0, y=0, z=180, z_speed=40).wait_for_completed() # turn around
            ep_chassis.move(x=0.3, y=0, z=0, xy_speed=0.5).wait_for_completed() # move forwards 30cm
            ep_chassis.move(x=0, y=0, z=-90, z_speed=40).wait_for_completed() # turn left
            ep_chassis.move(x=0.2, y=0, z=0, xy_speed=0.5).wait_for_completed() # move forwards 20cm
            release()
        else:
            resetArmPos()
            ep_chassis.move(x=0, y=0.3, z=0, xy_speed=0.5).wait_for_completed() # move right 20cm
            ep_chassis.move(x=0.3, y=0, z=0, xy_speed=0.5).wait_for_completed() # move forward 30cm
            findLine() # find blue line
            driveToLine()
            findLego(model)
            ...
            ep_chassis.move(x=0, y=0, z=180, z_speed=40).wait_for_completed() # turn around
            ep_chassis.move(x=0.3, y=0, z=0, xy_speed=0.5).wait_for_completed() # move forwards 30cm
            ep_chassis.move(x=0, y=0, z=90, z_speed=40).wait_for_completed() # turn right
            ep_chassis.move(x=0.2, y=0, z=0, xy_speed=0.5).wait_for_completed() # move forwards 20cm
            release()